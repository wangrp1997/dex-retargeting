...
def produce_frame(queue: multiprocessing.Queue, resolution: str = "full"):
    try:
        ...
        while True:
            ...
            if not ret:
                print("Failed to read frame from OAK-D camera")
                continue
            ...
            time.sleep(1 / 30.0)
            print("Before queue.put")  # 添加调试信息
            queue.put(frame)
            print("After queue.put")   # 添加调试信息
    except Exception as e:
        print(f"Error with OAK-D camera: {str(e)}")
        import traceback
        traceback.print_exc()
        raise
    finally:
...
def main(
    ...
    """
    Detects the human hand pose from OAK-D camera and translates the human pose trajectory into a robot pose trajectory.
    ...
    Args:
        ...
        resolution: Camera resolution, choose between "full" (1280x800) or "ultra" (3840x2160)
    """
    ...
    producer_process = multiprocessing.Process(
        target=produce_frame, args=(queue, resolution)
    )
