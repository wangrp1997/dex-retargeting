...
def main(
    ...
    cam.set_local_pose(sapien.Pose([0, 0.4, 0.2], [-0.707, 0, 0, 0.707]))
    # 创建查看器
    viewer = Viewer()
    viewer.set_scene(scene)
    viewer.control_window.show_origin_frame = False
    ...
    if "ability" in robot_name:
        ...
        loader.scale = 1.5
    elif "botyard" in robot_name:  # add botyard hand's urdf
        loader.scale = 0.9
    ...
    #     filepath = str(filepath)
    filepath = str(filepath)
    print(f"使用 URDF 文件: {filepath}")

    # 加载机器人
    robot = loader.load(filepath)
    if robot is None:
        raise RuntimeError(f"无法加载机器人 URDF: {filepath}")
