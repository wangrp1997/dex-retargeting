...
from dex_retargeting.src.oak import OakCamera  # 添加 OAK 相机导入
...
class RetargetingNode(Node):
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().info("OAK 相机初始化成功")
        except Exception as e:
            self.get_logger().error(f"OAK 相机初始化失败: {e}")
            raise
